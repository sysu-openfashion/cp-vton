...
Run the matlab code ```convert_data.m ``` (moved to data folder) under the original data root ```VITON/data```, and get the new format.

We use the json format for pose info as generated by [OpenPose](https://github.com/CMU-Perceptual-Computing-Lab/openpose) (18 keypoints version).
...
```
python test.py --name gmm_test_new --stage GMM --workers 4 --data_list test_pairs.txt --checkpoint checkpoints/gmm_train_new/gmm_final.pth
```
...
## Try-On Module
### training
Before the trainning, you should generate warp-mask & warp-cloth, using the test process of GMM. Then move these files or make soft links under the directory `data/train`.
An example training command is
...
```
python test.py --name tom_test_new --stage TOM --workers 4 --data_list test_pairs.txt --checkpoint checkpoints/tom_train_new/tom_final.pth
```
